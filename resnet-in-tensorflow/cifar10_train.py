...
class Train(object):
    ...
    def build_train_validation_graph(self):
        ...
        #inference is from resnet. It creates the resnet32
        ...
        #defining that total loss = softmax loss + regularizer loss
        ...
        #after defining all the loss and layers, defining how the training should go based on FULL LOSS and error
    ...
    def train(self):
        ...
        for step in range(FLAGS.train_steps):
            ...
            ##
            print(train_error_value) #just the number of wrong predictions/ batch size
    ...
    def train_operation(self, global_step, total_loss, top1_error):
        ...
        opt = tf.train.GradientDescentOptimizer(learning_rate = self.lr_placeholder) #lr_placeholder is directly lr itself.

        #compute gradient of the loss and calculate the frobenius norm of each gradient.
        #https://www.tensorflow.org/api_docs/python/tf/compat/v1/train/Optimizer

        grads_and_vars = opt.compute_gradients(total_loss) 
        #return list of all (gradient, variable) where 'gradient' is the gradient for the 'variable'

        gv_cgd_fn = [(self.Cgd_Fn(gv[0], gv[1]), gv[1]) for gv in grads_and_vars]
        #creating list of [ St, Wt] as displayed in eqn7 where each St = Wt + lambda( gt/ frobnorm of gt)

        train_op = opt.apply_gradients(gv_cgd_fn, global_step=global_step)
        #applies the given gradient to update weights by learning rate. accepts lists of (gradient, variable)
        #https://stackoverflow.com/questions/37921781/what-does-opt-apply-gradients-do-in-tensorflow

        #how the weights are updated - just showing But never use??
        g0_cgd_fn = grads_and_vars[0][0] #gradient of loss at current weight
        w0_cgd_fn = grads_and_vars[0][1] #current weight
        s0_cgd_fn = self.Cgd_Fn(g0_cgd_fn, w0_cgd_fn) # (W.t + lamb(g/ g.frob))
        w1_cgd_fn = w0_cgd_fn - self.lr_placeholder * s0_cgd_fn # W.t+1
