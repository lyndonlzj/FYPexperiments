...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_preloss, train_loss, train_error, train_regloss = train(train_loader, model, criterion, optimizer, epoch)
        lr_scheduler.step()
        ...
        df = pd.DataFrame(data= {'train_error': total_train_error, 'vali_error': total_vali_error, \
                            'train_preloss':total_train_preloss, 'train_loss':total_train_loss, 'train_regloss': total_train_regloss})
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        #loss w/o reg added is here
        loss = criterion(output, target_var)
        print(f'lost w/o reg: {loss.item()}')
        prelosses.update(loss.float().item(), input.size(0))
        ...
        with torch.no_grad():
            for param in model.parameters():
                if param.requires_grad:
                    ...
                    #check if torch.norm gives squared version or not. But we intend to use squared version for global reg.
                    #https://pytorch.org/docs/stable/generated/torch.norm.html

        #collect all reg_loss for all batches
        reg_loss.update(weight_reg.item(), input.size(0))
        ...
        if i % args.print_freq == 0:#only every 391 batches
            ...
            train_preloss.append(prelosses.val)

    return train_preloss, train_loss, train_error, train_regloss
